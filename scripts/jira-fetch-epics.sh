...
#
# Usage: bash scripts/jira-fetch-epics.sh
# Returns lines suitable for AskUserQuestion options

set -euo pipefail

# Determine toolbox root (script is in toolbox/scripts/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TOOLBOX_ROOT="${TOOLBOX_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"

# Load Jira environment
if [[ ! -f "$TOOLBOX_ROOT/private/jira.sh" ]]; then
  echo "Error: $TOOLBOX_ROOT/private/jira.sh not found. Run './scripts/jira-setup.sh' first." >&2
  exit 1
fi
